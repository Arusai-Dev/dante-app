...
function ChatBubble({ message, sender }: { message: string; sender: "user" | "assistant" }) {
  return (
    <div className={`flex items-start gap-3 ${sender === "user" ? "flex-row-reverse" : ""}`}>
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
          sender === "user" ? "bg-white text-neutral-900" : "bg-neutral-700 text-white"
        }`}
      >
...
export default function Sidebar({ onWidthChange }: SidebarProps) {
  ...
  useEffect(() => {
    if (allMessages.length > 0) {
      localStorage.setItem("sidebar-chat-history", JSON.stringify(allMessages))
    }
  }, [allMessages])
        ...
        {allMessages.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-neutral-400 max-w-xs mx-auto leading-relaxed">
              Ask me anything about your flashcards or any topic you&apos;d like to explore.
            </p>
          </div>
        )}
          ...
          <div className="flex items-start gap-3 animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="bg-neutral-800 rounded-2xl rounded-bl-md px-4 py-3 border border-neutral-700">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-neutral-300 ml-2">{selectedModel.name} is thinking...</span>
              </div>
            </div>
          </div>
        )}
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <div className="flex items-center space-x-1.5">
                      <div className={`w-1.5 h-1.5 rounded-full ${selectedModel.color} ${selectedModel.color === "bg-white" ? "border border-neutral-400" : ""}`} />
                    ...
                    >
                      <div className="flex items-center justify-between w-full mb-2">
                        <div className="flex items-center space-x-3">
                          <div
                            className={`w-2 h-2 rounded-full ${model.color} ${model.color === "bg-white" ? "border border-neutral-600" : ""}`}
                          />
                          <span className="font-medium text-white">{model.name}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Badge variant="secondary" className="bg-neutral-700 text-neutral-300 border-0 text-xs px-2 py-1">
                            {model.badge}
                          </Badge>
                          {selectedModel.id === model.id && (
                            <Badge className="bg-white text-neutral-900 border-0 text-xs px-2 py-1">Active</Badge>
                          )}
                        </div>
                      </div>
                      <span className="text-xs text-neutral-400 leading-relaxed">{model.description}</span>
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
