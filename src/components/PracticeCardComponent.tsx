...
export default function CardButton({ jsonCards, number_cards, setId }) {
    ...
    const [reviewQueue, setReviewQueue] = useState(jsonCards);
    ...
    const nextCard = async () => {
        ...
        const fsrsResult = await FsrsPatchAction(jsonCards[currentCard], qualityScore, setId);
        const dbCards = await getSetById(setId);
        const now = new Date();
        
        const newCards = dbCards[0].cards.filter(card => !card.stability);
        const dueCards = dbCards[0].cards.filter(card => card.stability && new Date(card.due) <= now);
        setReviewQueue([...newCards, ...dueCards]);
            ...
            <div className="h-screen w-screen flex items-center justify-center">
                ...
                <div className={`absolute w-1/2 h-1/2 transition-transform duration-500 flip-inner cursor-pointer ${!showFront ? 'flipped' : ''}`} onClick={flipCard} >
                    ...
                    <div className="flip-face front absolute top-0 left-0 w-full h-full bg-[#D9D9D9]/3 rounded-[10px] hover-animation">
                        <h2 className="pl-3 py-2"><Album className="inline-block mr-1" /> {reviewQueue[currentCard].category}</h2>
                        <div className="flex justify-center items-center h-[calc(100%-80px)] text-2xl">{reviewQueue[currentCard].front}</div>
                    ...
                    <div className="flip-face back absolute top-0 left-0 w-full h-full bg-[#D9D9D9]/6 rounded-[10px] hover-animation">
                        <h2 className="pl-3 py-2"><Album className="inline-block mr-1" /> {reviewQueue[currentCard].category}</h2>
                        <div className="flex justify-center items-center h-[calc(100%-80px)] text-2xl">{reviewQueue[currentCard].back}</div>
